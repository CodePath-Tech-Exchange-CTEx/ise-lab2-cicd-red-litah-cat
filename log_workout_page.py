...
def display_log_workout_page():
    ...
    with st.form("log_workout_form"):
        workout_type = st.selectbox(
            "Workout Type",
            ["Running", "Cycling", "Strength Training", "Swimming", "Yoga", "HIIT", "Walking", "Other"]
        )
        ...
        with col3:
            calories = st.number_input("Calories Burned (optional)", min_value=0, max_value=5000, value=0)
        with col4:
            workout_date = st.date_input("Date", value=datetime.date.today())

        notes = st.text_area("Notes (optional)", placeholder="How did the workout go?")

        submitted = st.form_submit_button("Save Workout", use_container_width=True)

        if submitted:
            st.success(
                f"Workout logged: {duration} min of {workout_type} at {intensity} intensity on {workout_date}."
            )
